#!/bin/bash
# SGFA qMAP-PD Remote Workstation Setup Script
# Handles environment setup, CUDA configuration, and quota checks
# Run experiments with: python run_experiments.py
...
check_directory() {
    if [ ! -f "requirements.txt" ] || [ ! -f "config.yaml" ]; then
        print_error "Error: Run this script from the sgfa_qmap-pd project root directory"
        print_info "Make sure you have requirements.txt and config.yaml in the current directory"
        exit 1
    fi
}
...
check_data() {
    ...
    if [ -f "$CONFIG_FILE" ]; then
        # Extract data directory from YAML config
        DATA_DIR=$(grep "data_dir:" "$CONFIG_FILE" | awk '{print $2}' | tr -d '"' | tr -d "'")
        ...
        if [ -d "$DATA_DIR" ]; then
            ...
            if [ ! -z "$NEW_DATA_DIR" ]; then
                if [ -d "$NEW_DATA_DIR" ]; then
                    # Update YAML config (simple sed replacement)
...
show_run_instructions() {
    ...
    echo "  source venv/bin/activate"
    echo "  nohup python run_experiments.py --experiments all > experiment.log 2>&1 &"
    echo ""
    ...
    echo "  python run_experiments.py --experiments data_validation"
    echo "  python run_experiments.py --experiments method_comparison"
    echo ""
    echo "${GREEN}Troubleshooting mode (independent data loading):${NC}"
    echo "  python run_experiments.py --experiments all --independent-mode"
    echo ""
