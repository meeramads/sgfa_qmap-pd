...
def get_advanced_preprocessing_data(config: Dict, data_dir: str,
                                   preprocessing_strategy: str = "standard") -> Tuple[List[np.ndarray], Dict]:
    """
    ...
    try:
        ...
        preprocessing_strategies = config.get('data_validation', {}).get('preprocessing_strategies', {})
        strategy_config = preprocessing_strategies.get(preprocessing_strategy, {})

        if not strategy_config:
            logger.warning(f"Preprocessing strategy '{preprocessing_strategy}' not found in config, using standard")
        ...
        view_names = basic_data.get('view_names', [f'view_{i}' for i in range(len(X_list_raw))])

        logger.info(f"Loaded data: {len(X_list_raw)} views with shapes {[X.shape for X in X_list_raw]}")
        ...
        if not strategy_config.get('enable_advanced_preprocessing', False):
            ...
            return _apply_advanced_preprocessing(X_list_raw, view_names, strategy_config, data_dir)
...
def _apply_minimal_preprocessing(X_list: List[np.ndarray], view_names: List[str],
                                strategy_config: Dict) -> Tuple[List[np.ndarray], Dict]:
...
def _apply_advanced_preprocessing(X_list: List[np.ndarray], view_names: List[str],
                                 strategy_config: Dict, data_dir: str) -> Tuple[List[np.ndarray], Dict]:
    """Apply advanced preprocessing using NeuroImagingPreprocessor."""
    try:
        from data.preprocessing import NeuroImagingPreprocessor, NeuroImagingConfig
        ...
        preprocessor = NeuroImagingPreprocessor(data_dir=data_dir, **preprocessing_config.to_dict())
        ...
        reduction_ratio = total_features_after / total_features_before if total_features_before > 0 else 1.0
        ...
        logger.info(f"   Feature reduction: {total_features_before} → {total_features_after} ({reduction_ratio:.3f} ratio)")
...
def get_optimal_preprocessing_strategy(config: Dict, X_list: List[np.ndarray],
    ...
    try:
        ...
        has_imaging_views = any('volume' in name.lower() or 'voxel' in name.lower()
        ...
        strategies = config.get('data_validation', {}).get('preprocessing_strategies', {})
        ...
        if avg_missing_pct > 20:
            ...
            optimal_strategy = 'aggressive'  # Feature selection needed for high-dimensional imaging
            ...
            reason = f"Small sample size ({n_subjects}) requires conservative preprocessing"
        else:
...
def apply_preprocessing_to_pipeline(config: Dict, data_dir: str,
                                   auto_select_strategy: bool = True,
                                   preferred_strategy: str = "standard") -> Tuple[List[np.ndarray], Dict]:
    """
    ...
    try:
        ...
        if auto_select_strategy:
            strategy_name, strategy_evaluation = get_optimal_preprocessing_strategy(config, X_list_raw, data_dir)
        else:
            strategy_name = preferred_strategy
            strategy_evaluation = {'selected_strategy': strategy_name, 'method': 'user_specified'}

        # Apply preprocessing
        X_processed, preprocessing_info = get_advanced_preprocessing_data(config, data_dir, strategy_name)
        ...
        comprehensive_info = {
            ...
            'data_summary': {
                ...
                'processed_shapes': [X.shape for X in X_processed],
                'view_names': basic_data.get('view_names', [f'view_{i}' for i in range(len(X_processed))])
            }
        }
