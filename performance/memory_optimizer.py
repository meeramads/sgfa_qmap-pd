...
class MemoryOptimizer:
    ...
    def __init__(self, 
        ...
        logger.info(f"MemoryOptimizer initialized: max_memory={self.max_memory_gb:.2f}GB")
    ...
    def optimize_array_memory(self, array: np.ndarray, target_dtype: str = None) -> np.ndarray:
        """
        ...
        if target_dtype is None:
            ...
            if array.dtype == np.float64:
                if np.allclose(array, array.astype(np.float32), rtol=1e-6):
                    ...
                    logger.debug(f"Optimized array from float64 to float32, saved {array.nbytes * 0.5 / 1024**2:.1f}MB")
        ...
        while self._monitoring_active:
            try:
                ...
                if pressure == 'critical':
                    logger.warning(f"CRITICAL: Memory usage at {usage['usage_fraction']:.1%}")
                    self.aggressive_cleanup()
                elif pressure == 'warning':
                    logger.warning(f"WARNING: Memory usage at {usage['usage_fraction']:.1%}")
...
def adaptive_batch_size(
    ...
    logger.debug(f"Adaptive batch size: {batch_size} (total={total_size}, "
...
def memory_limit_context(limit_gb: float):
    ...
    try:
        ...
        logger.info(f"Memory context completed: peak={report['peak_memory_gb']:.2f}GB, "
