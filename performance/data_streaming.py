...
class DataStreamer:
    ...
    def __init__(self, 
        ...
        logger.info(f"DataStreamer initialized: chunk_size={chunk_size}, "
                   f"memory_limit={memory_limit_gb}GB")

    def stream_array_chunks(self, 
                           arrays: List[np.ndarray], 
                           axis: int = 0) -> Iterator[List[np.ndarray]]:
        """
        ...
        if self.chunk_size is None:
            element_size = sum(arr.dtype.itemsize * np.prod(arr.shape) // arr.shape[axis] 
                             for arr in arrays)
    ...
    def stream_hdf5_dataset(self, 
                           filepath: Union[str, Path],
                           dataset_names: List[str],
                           chunk_axis: int = 0) -> Iterator[Dict[str, np.ndarray]]:
        """
        ...
        with h5py.File(filepath, 'r') as f:
            ...
            for name, dataset in datasets.items():
                if dataset.shape[chunk_axis] != chunk_dim:
                    raise ValueError(f"Dataset {name} has different size along axis {chunk_axis}")
            
            # Calculate chunk size
            if self.chunk_size is None:
                element_size = sum(
                    dataset.dtype.itemsize * np.prod(dataset.shape) // dataset.shape[chunk_axis]
            ...
            for start_idx in range(0, chunk_dim, chunk_size):
                ...
                for name, dataset in datasets.items():
                    if chunk_axis == 0:
                        ...
                        # General slicing
                        slice_obj = tuple(
                            slice(start_idx, end_idx) if i == chunk_axis else slice(None)
...
class ChunkedDataLoader:
    ...
    def load_multiview_data_chunked(self, 
                                   data: Dict[str, Any],
                                   chunk_subjects: int = None) -> Iterator[Dict[str, Any]]:
        """
    ...
    def save_chunked_data(self, 
                         data: Dict[str, Any],
                         base_path: Union[str, Path],
                         chunk_subjects: int = None,
                         format: str = 'hdf5') -> List[Path]:
        """
        ...
        with h5py.File(filepath, 'w') as f:
            ...
            if 'clinical' in chunk_data and chunk_data['clinical'] is not None:
                ...
                for col in chunk_data['clinical'].columns:
                    clinical_group.create_dataset(col, data=chunk_data['clinical'][col].values)
    ...
    def load_chunked_data(self, 
                         ...
                         chunk_indices: List[int] = None) -> Iterator[Dict[str, Any]]:
        """
...
def optimize_chunk_size_for_memory(
    ...
    bytes_per_subject = features_per_subject * np.dtype(dtype).itemsize * 1.3  # 30% overhead
    ...
    logger.debug(f"Optimal chunk size: {chunk_size} subjects "
