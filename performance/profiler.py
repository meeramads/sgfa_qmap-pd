...
class PerformanceProfiler:
    ...
    def compare_functions(self, function_names: List[str] = None) -> pd.DataFrame:
        ...
        if function_names:
            filtered_metrics = [
                m for m in self.metrics_history if m.function_name in function_names
            ]
        else:
...
def benchmark_function(
    ...
    **kwargs
    ...
    logger.info(
        f"Benchmark completed: {metrics.to_summary_string()} "
...
def profile_memory_usage(
    func: Callable = None, interval: float = 0.1, duration: float = None
):
    """
...
class ComparisonBenchmark:
    ...
    def add_benchmark(
        self, name: str, func: Callable, iterations: int = 5, **kwargs
    ) -> PerformanceMetrics:
        """
        ...
        **kwargs
        ...
        metrics = benchmark_function(func, iterations=iterations, **kwargs)
