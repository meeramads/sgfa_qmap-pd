...
class LatentClassAnalysisModel(BaseGFAModel):
    ...
    def __call__(self, X_list: List[jnp.ndarray]):
        ...
        with numpyro.plate("subjects", N):
            class_assignments = numpyro.sample("class_assignments", dist.Categorical(pi))
    ...
    def estimate_memory_requirements(self, N: int, D: int, K: int) -> Dict[str, float]:
        ...
        samples_memory = 1000 * (pi_memory + class_assignments_memory +
                                class_means_memory + class_vars_memory)

        total_bytes = (pi_memory + class_assignments_memory +
                      class_means_memory + class_vars_memory +
                      gradient_memory + samples_memory)

        return {
            'total_gb': total_bytes / (1024**3),
            'parameters_gb': (pi_memory + class_assignments_memory +
                            class_means_memory + class_vars_memory) / (1024**3),
