...
def get_optimal_model_configuration(config: Dict, data_characteristics: Dict = None) -> Tuple[Any, Dict]:
    """
    ...
    try:
        ...
        if data_characteristics:
            ...
            if model_config.get('auto_select', False):
                if has_imaging_data and total_features > 10000:
                    ...
                    reason = "High-dimensional data - sparsity regularization recommended"
                else:
                ...
                if model_config.get('use_recommendation', True):
                    ...
                    logger.info(f"⚠️  Recommendation ignored - using configured: {model_type}")
        ...
        model_configuration = {
            ...
            'configuration_strategy': 'auto_selected' if model_config.get('auto_select', False) else 'configured',
...
def create_model_instance(model_type: str, config: Dict, hypers: Dict,
                         data_characteristics: Dict = None) -> Tuple[Any, Dict]:
    """
...
def _extract_spatial_info(data_characteristics: Dict = None, config: Dict = None) -> Optional[Dict]:
    ...
    if imaging_views:
        ...
        logger.info(f"   Created basic spatial info for {len(imaging_views)} imaging views")
        return spatial_info

    return None


def apply_models_framework_to_pipeline(config: Dict, X_list: List[np.ndarray] = None,
                                     data_characteristics: Dict = None,
                                     hypers: Dict = None) -> Tuple[Any, Any, Dict]:
    """
...
def _create_default_hyperparameters(config: Dict, X_list: List[np.ndarray] = None) -> Dict:
    ...
    hypers = {
        ...
        'slab_scale': 2.0,
        'slab_df': 4.0,
        'percW': config.get('hyperparameter_optimization', {}).get('fallback_percW', 33)
    }
...
def run_model_comparison_analysis(available_models: List[str], config: Dict,
                                X_list: List[np.ndarray], hypers: Dict) -> Dict:
    """
    ...
    try:
        ...
        data_characteristics = {
            ...
            'has_imaging_data': any(X.shape[1] > 1000 for X in X_list) if X_list else False
        }

        logger.info(f"Comparing {len(available_models)} model types:")
        logger.info(f"   Models: {available_models}")
        logger.info(f"   Data: {data_characteristics['n_subjects']} subjects, {data_characteristics['total_features']:,} features")
            ...
            try:
                ...
                if model_instance:
                    model_result = {
                        ...
                        'spatial_info_used': creation_info.get('spatial_info_available', False),
                        'suitable_for_data': _assess_model_suitability(model_type, data_characteristics),
                        'recommended_use_cases': _get_model_use_cases(model_type)
                    }

                    logger.info(f"   ✅ {model_type}: {model_result['model_name']}")
                    logger.info(f"   Suitability: {model_result['suitable_for_data']['score']:.2f}")
                else:
                    model_result = {
                        'model_type': model_type,
                        'creation_successful': False,
                        'error': creation_info.get('error', 'unknown'),
                        'suitable_for_data': {'score': 0.0, 'reason': 'creation_failed'}
                    }
        ...
        return {
            ...
            'recommendation': {
                ...
                'reason': comparison_results.get(best_model, {}).get('suitable_for_data', {}).get('reason', 'highest_score') if best_model else 'no_suitable_model'
            }
        }
...
def integrate_models_with_pipeline(config: Dict, X_list: List[np.ndarray] = None,
                                 data_characteristics: Dict = None,
                                 hypers: Dict = None) -> Tuple[Any, Any, Dict]:
    """
    ...
    try:
        ...
        integration_summary = {
            ...
            'structured_model_management': framework_info.get('structured_model_management', False),
            ...
            'sparsity_regularization': framework_info.get('model_type') in ['sparseGFA', 'neuroGFA']
        }
        ...
        if framework_info.get('framework_available', False):
            try:
                ...
                if len(available_models) > 1 and config.get('model', {}).get('enable_comparison', False):
        ...
        if model_instance:
            ...
            logger.info(f"   Model type: {integration_summary.get('model_type_selected', 'unknown')}")
            logger.info(f"   Model factory: {integration_summary.get('model_factory_used', False)}")
            logger.info(f"   Structured management: {integration_summary.get('structured_model_management', False)}")
        else:
...
class ModelsFrameworkWrapper:
    ...
    def get_model_for_execution(self):
        ...
        if self.model_instance:
            ...
            logger.warning("No structured model available - falling back to core models")
            return None
    ...
    def get_model_info(self) -> Dict:
        ...
        return {
            ...
            'model_name': self.model_instance.get_model_name() if self.model_instance else 'none',
    ...
    def compare_with_alternatives(self, config: Dict, X_list: List[np.ndarray], hypers: Dict) -> Dict:
        """Compare current model with available alternatives."""
        try:
            from models import ModelFactory
            available_models = ModelFactory.list_models()

            return run_model_comparison_analysis(available_models, config, X_list, hypers)
        except Exception as e:
