...
def example_factor_visualization():
    ...
    print(f"🔄 Visualizing factors for {len(X_list)} views, {Z_true.shape[1]} factors...")
    ...
    for i, W in enumerate(W_true):
        ...
        visualizer.plot_loadings_heatmap(
            W,
            title=f"Factor Loadings - View {i+1}",
            ax=ax
        )
    ...
    for k in range(Z_true.shape[1]):
        visualizer.plot_factor_distribution(
            Z_true[:, k],
            title=f"Factor {k+1} Distribution",
            ax=axes[k]
        )
    ...
    for i, factor in enumerate(factors):
        ax2.annotate(factor, (stability_scores[i], interpretability_scores[i]),
    ...
    return {
        ...
        'n_views': len(X_list),
        'plots_created': ['loadings_heatmaps', 'scatter_plots', 'distributions', 'interpretability']
    }
...
def example_preprocessing_visualization():
    ...
    for i, X in enumerate(X_original):
        ...
        X_standardized = (X_imputed - np.mean(X_imputed, axis=0)) / np.std(X_imputed, axis=0)
    ...
    axes[0, 0].bar([f"View {v+1}" for v in views], missing_counts, color='orange', alpha=0.7)
    axes[0, 0].set_title('Missing Values Imputed')
    axes[0, 0].set_ylabel('Count')

    # Features removed
    axes[0, 1].bar([f"View {v+1}" for v in views], features_removed, color='red', alpha=0.7)
    ...
    axes[1, 0].bar(x + width/2, final_features, width, label='After Processing', alpha=0.7)
    ...
    axes[1, 1].bar([f"View {v+1}" for v in views], efficiency, color='green', alpha=0.7)
...
def example_brain_visualization():
    ...
    for k in range(n_factors):
        ...
        center_x, center_y, center_z = (brain_coords['x'][center_idx],
                                      brain_coords['y'][center_idx],
                                      brain_coords['z'][center_idx])

        # Calculate distances from center
        distances = np.sqrt((brain_coords['x'] - center_x)**2 +
    ...
    for k in range(n_factors):
        ...
        scatter = axes[0].scatter(brain_coords['x'], brain_coords['y'],
                               c=loadings, cmap='RdBu_r', s=20, alpha=0.7)
        ...
        scatter = axes[1].scatter(brain_coords['y'], brain_coords['z'],
                               c=loadings, cmap='RdBu_r', s=20, alpha=0.7)
        ...
        scatter = axes[2].scatter(brain_coords['x'], brain_coords['z'],
                               c=loadings, cmap='RdBu_r', s=20, alpha=0.7)
    ...
    for i in range(n_regions):
        for j in range(i+1, n_regions):
            distance = np.sqrt((brain_coords['x'][i] - brain_coords['x'][j])**2 +
    ...
    axes[1].scatter(brain_coords['x'], brain_coords['y'],
                   s=30, c='lightblue', alpha=0.6, edgecolors='black')
    ...
    for i in range(n_regions):
        for j in range(i+1, n_regions):
            if strong_connections[i, j]:
                ...
                axes[1].plot([brain_coords['x'][i], brain_coords['x'][j]],
...
def example_comprehensive_report():
    ...
    # Explained variance
    factors = [f"F{i+1}" for i in range(len(analysis_results['model_results']['explained_variance']))]
    ...
    for k in range(min(2, data['Z_true'].shape[1])):
        axes[1, 1].hist(data['Z_true'][:, k], bins=30, alpha=0.6,
                       label=f'Factor {k+1}')
    ...
    summary_stats = {
        'analysis_summary': {
            ...
            'validation_metrics': {
                'mean_cv_score': float(np.mean(analysis_results['validation_results']['cv_scores'])),
                'std_cv_score': float(np.std(analysis_results['validation_results']['cv_scores'])),
                'reproducibility': analysis_results['validation_results']['reproducibility']
            },
            'data_quality': analysis_results['preprocessing_info']
        },
        'factor_details': {
            f'factor_{i+1}': {
                'explained_variance': float(analysis_results['model_results']['explained_variance'][i]),
                'stability_score': float(analysis_results['validation_results']['stability_scores'][i])
            }
    ...
    return {
        ...
        'plots_created': ['performance_summary', 'quality_dashboard', 'html_report', 'summary_stats']
    }
...
def example_custom_plotting():
    ...
    for i, (angle, color) in enumerate(zip(angles, factor_colors)):
        ...
        ax.annotate(f'F{i+1}', (x, y), ha='center', va='center',
    ...
    scatter = ax.scatter(data['Z_true'][:, 0], data['Z_true'][:, 1],
    ...
    sns.heatmap(combined_loadings.T, cmap='RdBu_r', center=0,
                ax=ax_main, cbar_kws={'label': 'Loading Strength'})
    ax_main.set_title('Factor Loadings Across All Views', fontsize=16, fontweight='bold')
    ...
    for k in range(data['Z_true'].shape[1]):
        ax_dist.hist(data['Z_true'][:, k], bins=20, alpha=0.6,
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"• Brain visualizations: {brain_results['n_factors']} factors × {brain_results['n_regions']} regions")
        print(f"• Preprocessing plots: {len(preprocessing_results['plots_created'])} types")
        print(f"• Comprehensive report: {report_results['report_path']}")
        print(f"• Custom styling: {len(custom_results['plots_created'])} publication-ready plots")
        print(f"• All outputs saved to: {factor_results['output_dir']}")

        # List all created files
        output_dir = factor_results['output_dir']
        created_files = list(output_dir.glob("*.png")) + list(output_dir.glob("*.html")) + list(output_dir.glob("*.json"))
