...
def example_synthetic_data_generation():
    ...
    # Noise levels
    noise_levels = []
    for i, (X, W_true) in enumerate(zip(advanced_data['X_list'], advanced_data['W_true'])):
...
def example_advanced_preprocessing():
    ...
    for strategy in strategies:
        ...
        try:
            ...
            if 'strategy_selection' in preprocessing_info:
                ...
                print(f"   Selected strategy: {strategy_info.get('selected_strategy', 'unknown')}")
    ...
    for strategy, result in strategy_results.items():
        if result['success']:
            ...
            print(f"{strategy:<12} {'✅':<8} {len(result['X_list']):<6} {n_features:<16}")
        else:
...
def example_data_quality_assessment():
    ...
    for i, X in enumerate(X_list):
        ...
        view_metrics['mean_skewness'] = float(np.nanmean([
            abs(np.nanmean((X[:, j] - np.nanmean(X[:, j]))**3) / np.nanvar(X[:, j])**1.5)
            for j in range(X.shape[1]) if np.nanvar(X[:, j]) > 0
        ]))
    ...
    print(f"\n📋 DATA QUALITY REPORT")
    print("-" * 80)
    print(f"{'View':<6} {'Missing %':<10} {'Outliers %':<12} {'Constant':<10} {'Mean Var':<10} {'Skewness':<10}")
    print("-" * 80)

    for i in range(len(X_list)):
        metrics = quality_metrics[f'view_{i}']
        print(f"{i:<6} {metrics['missing_percentage']:<10.2f} "
    ...
    for i in range(len(X_list)):
        ...
        if metrics['missing_percentage'] > 5:
            print(f"  ⚠️  High missing values ({metrics['missing_percentage']:.1f}%) - consider advanced imputation")

        if metrics['outlier_percentage'] > 2:
            print(f"  ⚠️  High outlier rate ({metrics['outlier_percentage']:.1f}%) - consider robust methods")

        if metrics['constant_features'] > 0:
            print(f"  ⚠️  {metrics['constant_features']} constant features - should be removed")

        if metrics['mean_skewness'] > 2:
            print(f"  ⚠️  High skewness ({metrics['mean_skewness']:.2f}) - consider transformation")
...
def example_data_splitting_and_cv():
    ...
    clinical_labels = np.random.choice(['Control', 'PD_Mild', 'PD_Severe'],
                                     size=n_subjects,
                                     p=[0.4, 0.35, 0.25])
    ...
    print(f"  Label distribution: {dict(zip(*np.unique(clinical_labels, return_counts=True)))}")
    ...
    print(f"  Train: {len(train_idx)} subjects ({len(train_idx)/n_subjects*100:.1f}%)")
    print(f"  Validation: {len(val_idx)} subjects ({len(val_idx)/n_subjects*100:.1f}%)")
    ...
    for fold, (train_idx, val_idx) in enumerate(stratified_kfold.split(range(n_subjects), clinical_labels)):
    ...
    for fold, (train_idx, val_idx) in enumerate(group_kfold.split(range(n_subjects), groups=subject_groups)):
...
def example_data_export_and_formats():
    ...
    try:
        ...
        with h5py.File(export_dir / "complete_dataset.h5", 'w') as f:
            ...
            for i, W in enumerate(W_true):
                true_group.create_dataset(f'loadings_view_{i}', data=W, compression='gzip')
