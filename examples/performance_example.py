...
def example_basic_usage():
    ...
    print(f"Auto-configured for system: {config.memory.max_memory_gb:.1f}GB memory limit")
...
def example_data_chunking():
    ...
    print(f"Generated data: {synthetic_data['X_list'][0].shape[0]} subjects, "
    ...
    total_processed = 0
    for chunk_id, chunk_data in enumerate(loader.load_multiview_data_chunked(
        synthetic_data, chunk_subjects=50
    )):
        chunk_size = chunk_data['X_list'][0].shape[0]
        total_processed += chunk_size
        print(f"Processed chunk {chunk_id}: {chunk_size} subjects "
...
def example_configuration_management():
    ...
    for config_name, config in configs.items():
        if config_name == 'memory_efficient':
            print(f"{'Max Memory (GB)':<30} {config.memory.max_memory_gb:<12.1f}", end="")
    ...
    for config_name, config in configs.items():
        if config_name == 'memory_efficient':
            print(f"{'Enable Compression':<30} {str(config.data.enable_compression):<12}", end="")
        elif config_name == 'balanced':
    ...
    for config_name, config in configs.items():
        if config_name == 'memory_efficient':
            print(f"{'Enable Profiling':<30} {str(config.profiling.enable_profiling):<12}", end="")
        elif config_name == 'balanced':
    ...
    print(f"Loaded configuration with {loaded_config.memory.max_memory_gb:.1f}GB memory limit")
...
def example_benchmarking():
    ...
    detailed_metrics = benchmark_function(matrix_multiply, iterations=10, warmup_iterations=2)
