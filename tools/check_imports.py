...
class ImportChecker:
    ...
    def _find_project_modules(self):
        ...
        for root, dirs, files in os.walk(self.project_root):
            ...
            if any(
                part in [".git", "__pycache__", ".pytest_cache", "experiment_results"]
    ...
    def suggest_fixes(self, broken_import: Dict) -> List[str]:
        ...
        if module_name in fixes:
            suggestions.append(
                f"Change 'from {module_name}' to 'from {fixes[module_name]}'"
            )
...
def main():
    ...
    # Summary
    success_rate = (
        (results["successful_imports"] / results["total_imports"] * 100)
