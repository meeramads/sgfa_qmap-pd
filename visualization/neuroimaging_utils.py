...
try:
    ...
    logging.warning(
        "nibabel not available - install with 'pip install nibabel' for full neuroimaging reconstruction features"
    )
...
class FactorToMRIMapper:
    ...
    def __init__(self, base_dir: str, reference_mri: str = None):
        ...
        if not NEUROIMAGING_AVAILABLE:
            logger.error(
                "nibabel is required for FactorToMRIMapper. Install with: pip install nibabel"
            )
            raise ImportError(
                "nibabel is required for neuroimaging reconstruction features"
            )
        ...
        for subject_id in subject_ids:
            ...
            for roi_name in self.roi_names:
                if roi_name in self.volume_files and roi_name in self.position_files:
                    output_file = (
                        output_dir / f"subject_{subject_id:02d}_{roi_name}.nii"
                    )
    ...
    def _reconstruct_roi_single(
        ...
        try:
            ...
            if V.shape[1] != len(pos):
                raise ValueError(
                    f"Volume columns ({V.shape[1]}) != position rows ({len(pos)})"
                )

            if subject_id < 1 or subject_id > V.shape[0]:
                raise ValueError(
                    f"Subject ID {subject_id} out of range [1, {V.shape[0]}]"
                )
            ...
            logging.error(
                f"Failed to reconstruct {vol_file.stem} for subject {subject_id}: {e}"
            )
            return None

    def _create_overlay_plot(
        self, roi_images: List[Tuple[str, np.ndarray]], output_file: Path
    ):
        """
        ...
        try:
            ...
            for i, view_axis in enumerate(["sagittal", "coronal", "axial"]):
                ...
                for j, (roi_name, roi_data) in enumerate(roi_images):
                    ...
                    if np.any(mask):
                        axes[i + 3].imshow(
                            roi_slice.T,
                            cmap=roi_colors[j % len(roi_colors)],
                            alpha=0.6,
                            origin="lower",
                        )
            ...
            legend_elements = [
                Patch(
                    facecolor=plt.cm.get_cmap(roi_colors[i % len(roi_colors)])(0.7),
                    label=roi_name,
                )
            ...
            plt.suptitle(
                f"Subject Reconstruction Overview", fontsize=16, fontweight="bold"
            )
            plt.tight_layout()
    ...
    def batch_reconstruct_subjects(
        self,
        n_subjects: int = None,
        subject_range: Tuple[int, int] = None,
        output_dir: str = "batch_reconstructions",
    ) -> Dict:
        """
        ...
        reconstructions = self.reconstruct_subject_data(
            subject_ids=subject_ids, output_dir=output_dir, create_overlays=True
        )
        ...
        stats = {
            ...
            "total_files_created": sum(
                len(files) for files in reconstructions.values()
            ),
            "processing_time_seconds": elapsed_time,
            "success_rate": (
                len(reconstructions) / len(subject_ids) if subject_ids else 0
            ),
        ...
        logging.info(
            f"  Subjects: {stats['n_subjects_completed']}/{stats['n_subjects_requested']}"
        )
        ...
        if "imaging" in view_name.lower() and len(self.roi_names) > 1:
            ...
            if roi_name and roi_name in self.position_files:
                ...
                if len(loadings) == len(positions):
                    ...
                    logger.warning(
                        f"Loading dimension mismatch for {view_name}: "
    ...
    def _create_roi_nifti(
        ...
        try:
            ...
            if not np.all(valid_indices):
                logger.warning(
                    f"Some position indices for {roi_name} are out of bounds"
                )
            ...
            output_file = (
                output_dir / f"factor_{factor_idx + 1}_{roi_name}_loadings.nii"
            )

            # Create and save NIfTI
            output_nii = nib.Nifti1Image(
                output_img, ref_img.affine, ref_img.header.copy()
            )
            output_nii.header.set_data_dtype(np.float32)
            output_nii.header["descrip"] = (
                f"Factor {factor_idx + 1} loadings for {roi_name}"
            )
            ...
            logger.info(
                f"  Value range: [{np.min(weights):.4f}, {np.max(weights):.4f}]"
            )
    ...
    def map_all_factors(
        ...
        logger.info(
            f"Generated {sum(len(outputs) for outputs in all_outputs.values())} NIfTI files"
        )
...
def integrate_subject_reconstructions_with_factors(
    results_dir: str,
    data: Dict,
    W: np.ndarray,
    base_dir: str,
...
def integrate_with_visualization(
    results_dir: str,
    data: Dict,
    W: np.ndarray,
    base_dir: str,
    ...
    if not NEUROIMAGING_AVAILABLE:
        logger.error(
            "nibabel is required for neuroimaging reconstruction. Install with: pip install nibabel"
        )
        raise ImportError(
            "nibabel is required for integrate_with_visualization function"
        )
    ...
    for factor_idx in factor_indices:
        ...
        factor_maps = mapper.map_weights_to_mri(
            W, view_names, Dm, factor_idx, output_dir=str(factor_maps_dir)
        )
    ...
    with open(summary_file, "w") as f:
        ...
        f.write(
            f"Total NIfTI files created: {sum(len(maps) for maps in all_factor_maps.values())}\n"
        )
...
def add_to_qmap_visualization(
    data: Dict, res_dir: str, args, hypers, base_dir: str = None, brun: int = None
):
    """
...
def create_factor_overlay_images(
    factor_maps: Dict[int, Dict[str, str]],
    reference_mri: str,
    output_dir: str = "overlay_images",
):
    """
    ...
    for factor_idx, roi_maps in factor_maps.items():
        ...
        combined_nii = nib.Nifti1Image(
            combined_img, ref_img.affine, ref_img.header.copy()
        )
...
def extract_top_voxels(
    factor_maps: Dict[int, Dict[str, str]], top_k: int = 100
...
def example_usage():
    ...
    # Map first factor
    factor_maps = mapper.map_weights_to_mri(
        W_example, view_names, Dm, factor_idx=0, output_dir="example_factor_maps"
    )
