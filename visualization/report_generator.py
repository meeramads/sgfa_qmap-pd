...
class ReportGenerator:
    ...
    def generate_html_report(self, plot_dir: Path, 
                            data: Dict,
    ...
    def _create_cv_section(self, cv_results: Dict) -> str:
        ...
        if 'fold_results' in cv_results:
            converged = sum(1 for r in cv_results['fold_results'] 
                          if r.get('converged', False))
            content += f"<strong>Converged Folds:</strong> {converged}/{len(cv_results['fold_results'])}<br>"
            
        content += f"<strong>Total Time:</strong> {cv_results.get('total_time', 0):.1f}s<br>"
        content += "</div>"
