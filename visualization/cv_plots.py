...
class CrossValidationVisualizer:
    ...
    def plot_cv_results(self, cv_results: Dict, plot_path: str, run_name: str = "cross_validation"):
        """
        ...
        if 'hyperparameter_optimization' in cv_results:
            self._plot_hyperparameter_optimization(cv_results['hyperparameter_optimization'], plot_path)
    ...
    def _plot_cv_performance_summary(self, cv_results: Dict, plot_path: Path, run_name: str):
        """Plot overall CV performance metrics."""
        fig, axes = plt.subplots(2, 2, figsize=(15, 10))
        fig.suptitle(f'Cross-Validation Performance Summary: {run_name}', fontsize=16, fontweight='bold')
        ...
        if 'fold_scores' in cv_results:
            ...
            # Performance trend
            axes[0, 1].plot(range(1, len(fold_scores) + 1), fold_scores, 'bo-', linewidth=2, markersize=6)
            axes[0, 1].axhline(y=np.mean(fold_scores), color='r', linestyle='--', alpha=0.7, label=f'Mean: {np.mean(fold_scores):.4f}')
            axes[0, 1].fill_between(range(1, len(fold_scores) + 1), 
                                  np.mean(fold_scores) - np.std(fold_scores),
                                  np.mean(fold_scores) + np.std(fold_scores),
                                  alpha=0.2, color='red', label=f'±1 SD: {np.std(fold_scores):.4f}')
        ...
        if 'model_complexity' in cv_results:
            ...
            axes[1, 0].bar(range(len(complexity)), complexity, alpha=0.7, color='skyblue')
        ...
        if 'train_scores' in cv_results and 'val_scores' in cv_results:
            ...
            axes[1, 1].plot(folds, cv_results['train_scores'], 'go-', label='Training', linewidth=2)
            axes[1, 1].plot(folds, cv_results['val_scores'], 'ro-', label='Validation', linewidth=2)
    ...
    def _plot_factor_stability_heatmap(self, fold_results: Dict, plot_path: Path):
        ...
        n_factors = max([len(fold_results[f].get('factor_loadings', [])) for f in fold_results])
    ...
    def _plot_spatial_coherence(self, cv_results: Dict, plot_path: Path):
        ...
        if 'mean' in spatial_scores:
            ...
            axes[0].bar(['Overall'], coherence_values, yerr=err_values, 
                       capsize=10, alpha=0.7, color='lightblue')
    ...
    def _plot_subtype_validation(self, subtype_results: Dict, plot_path: Path):
        ...
        if 'silhouette_scores' in subtype_results:
            ...
            if isinstance(silhouette_data, dict) and 'mean' in silhouette_data:
                axes[0, 0].bar(['Mean Silhouette'], [silhouette_data['mean']], 
        ...
        if 'subtype_stability' in subtype_results:
            ...
            axes[0, 1].bar(range(1, n_subtypes + 1), stability_scores, 
        ...
        if 'clinical_associations' in subtype_results:
            ...
            if isinstance(clinical_data, dict):
                ...
                p_values = [-np.log10(clinical_data[var].get('p_value', 1.0)) for var in variables]
    ...
    def _plot_centroids_heatmap(self, C: np.ndarray, plot_path: Path):
        ...
        # Create heatmap
        sns.heatmap(C.T, annot=False, cmap='RdBu_r', center=0,
    ...
    def _plot_centroids_radar(self, C: np.ndarray, plot_path: Path):
        ...
        for i in range(n_subtypes):
            ...
            ax.plot(angles, values, 'o-', linewidth=2, 
                   label=f'Subtype {i+1}', color=colors[i])
            ax.fill(angles, values, alpha=0.25, color=colors[i])
        
        ax.set_xticks(angles[:-1])
        ax.set_xticklabels([f'Factor {i+1}' for i in range(n_factors)])
        ax.set_title('Subtype Centroids Radar Chart', size=14, fontweight='bold', y=1.08)
    ...
    def _plot_subtype_probabilities(self, prob_data: Dict, plot_path: Path):
        ...
        for i in range(n_subtypes):
            axes[0, 0].hist(prob_df.iloc[:, i], bins=30, alpha=0.7, 
                           label=f'Subtype {i+1}', density=True)
        ...
        axes[1, 0].bar(range(len(assignment_counts)), assignment_counts.values, 
        ...
        axes[1, 0].set_xticklabels([f'Subtype {i+1}' for i in assignment_counts.index])
        
        # Assignment certainty
        certainty = 1 - (prob_df.apply(lambda row: np.sum(np.sort(row)[-2:]), axis=1) - prob_df.max(axis=1))
    ...
    def _plot_subtype_assignments(self, prob_data: Dict, plot_path: Path):
        ...
        for subtype in range(prob_df.shape[1]):
            ...
            plt.scatter(np.where(mask)[0], max_probs[mask], 
                       c=[colors[subtype]], label=f'Subtype {subtype+1}', 
                       alpha=0.6, s=50)
