...
class BrainVisualizer:
    ...
    def create_brain_visualization_summary(self, results_dir: str, include_reconstructions: bool = True) -> Dict:
        """
        ...
        try:
            ...
            if self.factor_mapping_available:
                factor_maps = self._create_factor_brain_maps(results_dir, brain_plots_dir)
                summary['factor_maps'] = factor_maps
            
            # 2. Spatial coherence analysis
            spatial_analysis = self._analyze_spatial_coherence(results_dir, brain_plots_dir)
            summary['spatial_analysis'] = spatial_analysis
            
            # 3. Region-wise factor analysis
            region_analysis = self._create_region_factor_analysis(results_dir, brain_plots_dir)
            summary['region_analysis'] = region_analysis
            
            # 4. Subject reconstructions if requested
            if include_reconstructions:
                reconstruction_summary = self._create_reconstruction_summary(results_dir, brain_plots_dir)
            ...
            for factor_idx in range(n_maps_to_create):
                try:
                    ...
                    factor_maps[f'factor_{factor_idx + 1}'] = {
                        ...
                        'sparsity': float(np.sum(np.abs(factor_loadings) > 0.01) / len(factor_loadings))
                    }
                    
                except Exception as e:
                    logger.warning(f"Failed to create brain map for factor {factor_idx + 1}: {e}")
                    continue
    ...
    def _plot_factor_loading_distribution(self, loadings: np.ndarray, factor_idx: int, plot_dir: Path):
        """Plot distribution of factor loadings."""
        fig, axes = plt.subplots(2, 2, figsize=(15, 10))
        fig.suptitle(f'Factor {factor_idx + 1} Loading Analysis', fontsize=16, fontweight='bold')
        
        # Distribution of loadings
        axes[0, 0].hist(loadings, bins=50, alpha=0.7, color='skyblue', edgecolor='black')
        ...
        # Absolute loadings
        axes[0, 1].hist(np.abs(loadings), bins=50, alpha=0.7, color='lightcoral', edgecolor='black')
        ...
        axes[1, 0].barh(range(len(top_loadings)), top_loadings, 
                       color=['red' if x < 0 else 'blue' for x in top_loadings], alpha=0.7)
        ...
        axes[1, 1].plot(range(1, len(cumulative_loadings) + 1), cumulative_loadings, 'b-', linewidth=2)
        ...
        plt.tight_layout()
        plt.savefig(plot_dir / f'factor_{factor_idx + 1}_loading_analysis.png', 
            ...
            if preprocessing_results and 'metadata' in preprocessing_results:
                ...
                if metadata.get('spatial_processing_applied', False):
                    ...
                    spatial_analysis['spatial_processing'] = {
                        'applied': True,
                        'position_lookups_loaded': metadata.get('position_lookups_loaded', []),
                        'harmonization_applied': metadata.get('harmonization_applied', False)
                    }
    ...
    def _plot_spatial_processing_summary(self, metadata: Dict, plot_dir: Path):
        ...
        if 'feature_reduction' in metadata:
            ...
            original_features = [metadata['feature_reduction'][v]['original'] for v in views]
            processed_features = [metadata['feature_reduction'][v]['processed'] for v in views]
            
            x = np.arange(len(views))
            width = 0.35
            
            axes[0, 0].bar(x - width/2, original_features, width, label='Original', alpha=0.7)
            axes[0, 0].bar(x + width/2, processed_features, width, label='Processed', alpha=0.7)
        ...
        # Reduction ratios
        if 'feature_reduction' in metadata:
            reduction_ratios = [metadata['feature_reduction'][v]['reduction_ratio'] for v in views]
        ...
        if metadata.get('position_lookups_loaded'):
            ...
            axes[1, 0].bar(range(len(position_views)), [1]*len(position_views), 
        ...
        axes[1, 1].text(0.1, 0.7, f"Spatial Processing: {'Applied' if metadata.get('spatial_processing_applied') else 'Not Applied'}", 
                        fontsize=12, fontweight='bold', transform=axes[1, 1].transAxes)
        axes[1, 1].text(0.1, 0.5, f"Scanner Harmonization: {'Applied' if metadata.get('harmonization_applied') else 'Not Applied'}", 
                        fontsize=12, transform=axes[1, 1].transAxes)
        axes[1, 1].text(0.1, 0.3, f"Position Data Views: {len(metadata.get('position_lookups_loaded', []))}", 
                        fontsize=12, transform=axes[1, 1].transAxes)
    ...
    def _basic_spatial_analysis(self, results_dir: Path, plot_dir: Path) -> Dict:
        ...
        ax.text(0.5, 0.5, 'Basic Spatial Analysis\n(Full spatial processing not applied)', 
                ha='center', va='center', fontsize=14, fontweight='bold',
                transform=ax.transAxes)
        ax.text(0.5, 0.3, 'For enhanced spatial analysis, enable spatial processing\nwith --enable_spatial_processing', 
    ...
    def _create_region_factor_analysis(self, results_dir: Path, plot_dir: Path) -> Dict:
        ...
        try:
            ...
            fig, ax = plt.subplots(1, 1, figsize=(10, 6))
            ax.text(0.5, 0.5, 'Region-wise Factor Analysis\n(Requires region-specific data organization)', 
                    ha='center', va='center', fontsize=14, fontweight='bold',
                    transform=ax.transAxes)
            ax.set_xlim(0, 1)
            ax.set_ylim(0, 1)
    ...
    def _create_reconstruction_summary(self, results_dir: Path, plot_dir: Path) -> Dict:
        ...
        if reconstruction_dir.exists():
            ...
            reconstruction_summary['n_subjects_reconstructed'] = len(reconstruction_files)
            
            # Create reconstruction summary plot
            fig, axes = plt.subplots(2, 2, figsize=(15, 10))
            fig.suptitle('Subject Reconstruction Summary', fontsize=16, fontweight='bold')
            
            # Number of reconstructions
            axes[0, 0].bar(['Reconstructed'], [len(reconstruction_files)], 
            ...
            if reconstruction_files:
                file_sizes = [f.stat().st_size / 1024 / 1024 for f in reconstruction_files[:50]]  # MB, limit to 50
            ...
            axes[1, 0].text(0.5, 0.5, f'Reconstructions Created\n{len(reconstruction_files)} subjects', 
                           ha='center', va='center', fontsize=14, fontweight='bold',
...
"""
    ...
    def _create_brain_analysis_report(self, summary: Dict, plot_dir: Path):
        ...
        spatial_status = 'Applied' if summary.get('spatial_analysis', {}).get('spatial_processing', {}).get('applied', False) else 'Basic'
        status_colors = {'Applied': 'green', 'Basic': 'orange'}
        axes[0, 1].bar(['Spatial Analysis'], [1], alpha=0.7, color=status_colors.get(spatial_status, 'gray'))
        axes[0, 1].set_title('Spatial Processing Status')
        axes[0, 1].set_ylabel('Status')
        axes[0, 1].text(0, 0.5, spatial_status, ha='center', va='center', fontweight='bold')
        
        # Reconstruction summary
        n_reconstructions = summary.get('reconstruction_summary', {}).get('n_subjects_reconstructed', 0)
        axes[1, 0].bar(['Reconstructions'], [n_reconstructions], alpha=0.7, color='lightgreen')
...
"""
        ...
        plt.tight_layout()
        plt.savefig(plot_dir / 'brain_analysis_overview.png', dpi=300, bbox_inches='tight')
        plt.close()
