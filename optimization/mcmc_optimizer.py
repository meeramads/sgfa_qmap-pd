...
class MCMCMemoryOptimizer:
    ...
    def optimize_mcmc_config(
        ...
        if estimated_memory > self.memory_limit_gb:
            ...
            if memory_ratio < 0.3:
                ...
                logger.info(
                    f"Enabling data subsampling with ratio {self.subsample_ratio}"
                )
    ...
    def _estimate_mcmc_memory(
        self, n_subjects: int, n_features: int, n_factors: int
    ) -> float:
        ...
        param_memory = (
            z_memory + w_memory + sigma_memory
        ) * 2  # Current + proposed states
    ...
    def _subsample_data(
        self, X_list: List[np.ndarray], ratio: float
    ) -> List[np.ndarray]:
    ...
    def run_memory_efficient_mcmc(
        ...
        try:
            ...
            mcmc = MCMC(
                ...
                num_warmup=optimized_config.get(
                    "num_warmup", optimized_config["num_samples"] // 2
                ),
                chain_method=(
                    "parallel" if optimized_config["num_chains"] > 1 else "sequential"
                ),
            )
    ...
    def _thin_samples(
        self, samples: Dict[str, np.ndarray], interval: int
    ) -> Dict[str, np.ndarray]:
        ...
        logger.info(
            f"Thinned samples by factor {interval}: "
            f"{value.shape[0]} -> {thinned[key].shape[0]} samples"
        )
        return thinned

    def adaptive_batch_mcmc(
        self,
        model_fn: Callable,
        X_list: List[np.ndarray],
        hypers: Dict[str, Any],
        ...
        if not self.enable_batch_sampling:
            return self.run_memory_efficient_mcmc(
    ...
    def _aggregate_batch_samples(
        self, batch_samples: List[Dict[str, np.ndarray]]
    ) -> Dict[str, np.ndarray]:
        ...
        for key in sample_keys:
            if key in ["Z", "W"]:  # Parameters we can average
                ...
                batch_weights = [
                    samples[key].shape[1] if key == "Z" else samples[key].shape[1]
...
class GradientCheckpointer:
    ...
    def memory_efficient_scan(
        fn: Callable, init: Any, xs: Any, checkpoint_every: int = 10
    ) -> Tuple[Any, Any]:
        """
        ...
        xs_with_idx = (
            xs,
            jnp.arange(xs.shape[0] if hasattr(xs, "shape") else len(xs)),
        )
...
def memory_efficient_matrix_multiply(
    A: jnp.ndarray, B: jnp.ndarray, chunk_size: int = 1000
) -> jnp.ndarray:
    """
...
def memory_efficient_log_likelihood(
    X: jnp.ndarray,
    Z: jnp.ndarray,
    W: jnp.ndarray,
    sigma: jnp.ndarray,
    chunk_subjects: int = 100,
) -> jnp.ndarray:
    """
