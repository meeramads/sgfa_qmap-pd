...
def qmap_pd(data: Dict, res_dir: str, args: Any, hypers: Dict, topk: int = 20):
    ...
    try:
        ...
        view_names = data.get('view_names', [f'View_{i+1}' for i in range(len(X_list))])
...
def plot_preprocessing_summary(preprocessing_results: Dict, plot_path: str, view_names: List[str]):
    """Delegate to preprocessing visualizer."""
    config = VisualizationConfig()
    prep_viz = PreprocessingVisualizer(config)
    return prep_viz.plot_preprocessing_summary(preprocessing_results, plot_path, view_names)


def plot_cv_results(cv_results: Dict, plot_path: str, run_name: str = "cross_validation"):
...
def plot_consensus_subtypes(centroids_data: Dict, probabilities_data: Dict, plot_path: str):
    """Delegate to CV visualizer."""
    config = VisualizationConfig()
    cv_viz = CrossValidationVisualizer(config)
...
def create_brain_visualization_summary(results_dir: str, include_reconstructions: bool = True):
    ...
    config = VisualizationConfig()
    brain_viz = BrainVisualizer(config)
    return brain_viz.create_brain_visualization_summary(results_dir, include_reconstructions)


def create_all_visualizations(results_dir: str, data: Dict, run_name: str, 
...
def find_bestrun(res_dir: str, args: Any, ofile: str = None) -> int:
    ...
    for run_id in range(1, args.num_runs + 1):
        ...
        mcmc_samples = safe_pickle_load(files['model_params'], f"MCMC samples run {run_id}")
...
def _plot_ground_truth_components(true_params: Dict, plot_path: Path, args: Any, hypers: Dict):
...
def _plot_inferred_components(rob_params: Dict, true_params: Dict, plot_path: Path, args: Any, hypers: Dict):
...
def _plot_factor_comparison(true_params: Dict, rob_params: Dict, plot_path: Path, args: Any):
    """Compare true vs inferred factors."""
    config = VisualizationConfig()
    factor_viz = FactorVisualizer(config)
    
    if all(key in true_params for key in ['W_true', 'Z_true']) and all(key in rob_params for key in ['W', 'Z']):
        factor_viz.plot_factor_comparison(
            true_params['W_true'], rob_params['W'],
            true_params['Z_true'], rob_params['Z'],
            save_path=plot_path / 'factor_comparison.png'
        )


def _plot_subgroup_analysis(true_params: Dict, rob_params: Dict, plot_path: Path, args: Any):
...
def _plot_multiview_loadings(W: np.ndarray, Dm: List[int], view_names: List[str], 
                           feat_names: Dict, plot_path: Path, topk: int):
    """Plot multi-view factor loadings."""
    config = VisualizationConfig()
    factor_viz = FactorVisualizer(config)
    
    factor_viz.plot_multiview_loadings(
        W, Dm, view_names, feat_names, topk,
...
def _plot_subject_scores(Z: np.ndarray, sub_ids: List, plot_path: Path):
    ...
    factor_viz.plot_factor_scores(
        Z, subject_ids=sub_ids,
        save_path=plot_path / 'subject_scores.png'
    )


def _plot_latent_factor_summary(W: np.ndarray, Z: np.ndarray, Dm: List[int], 
                               view_names: List[str], plot_path: Path):
    ...
    factor_viz.plot_factor_summary(
        W, Z, Dm, view_names,
        save_path=plot_path / 'latent_factor_summary.png'
    )
