            ...
            for param_name, rule in validation_rules.items():
                if param_name in bound_args.arguments:
                    ...
                    try:
                        ...
                        raise ValueError(f"Parameter validation error for '{param_name}': {e}")
...
class ParameterValidator:
    ...
    def validate_range(value: Union[int, float], min_val: Optional[float] = None,
                      max_val: Optional[float] = None, name: str = "parameter") -> None:
    ...
    def validate_array_shape(array: np.ndarray, expected_shape: Optional[tuple] = None,
        ...
        if max_dims is not None and array.ndim > max_dims:
            raise ValueError(f"{name} must have at most {max_dims} dimensions, got {array.ndim}")
    ...
    @staticmethod
    def validate_dict_keys(dct: Dict, required_keys: List[str], optional_keys: Optional[List[str]] = None,
                          name: str = "dictionary") -> None:
...
class ResultValidator:
    ...
    @staticmethod
    def validate_experiment_result(result: Dict, required_fields: List[str] = None) -> None:
        """
        ...
        if not isinstance(result, dict):
            raise TypeError(f"Experiment result must be a dictionary, got {type(result)}")
        ...
        if missing_fields:
            raise ValueError(f"Experiment result missing required fields: {missing_fields}")
    ...
    @staticmethod
    def validate_mcmc_samples(samples: Dict, expected_keys: Optional[List[str]] = None) -> None:
        """
        ...
        for key, value in samples.items():
            if not isinstance(value, np.ndarray):
                raise TypeError(f"Sample '{key}' must be numpy array, got {type(value)}")
        ...
        for key, expected_type in sch.items():
            ...
            if isinstance(expected_type, dict):
                ...
                if not isinstance(value, dict):
                    warnings.append(f"Config key {current_path} should be dict, got {type(value)}")
                else:
