...
def main(args):
    ...
    try:
        ...
        if available_gb < 8:
            logging.error("Insufficient memory (<8GB available). Analysis likely to fail.")
            if not getattr(args, 'force_run', False):
                sys.exit(1)
        elif available_gb < 16:
            logging.warning("Limited memory (<16GB). Consider using --quick_cv or reducing parameters.")
    ...
    if config.run_cv:
        ...
        if cv_result:
            ...
            _save_cv_results(cv_results, cv_object, config.cv_res_dir, config_manager.dependencies)
    ...
    if config.run_standard:
        _create_brain_visualizations(args, config.standard_res_dir, data, config_manager.dependencies)
...
def _save_cv_results(cv_results, cv_object, cv_res_dir, dependencies):
    ...
    try:
        if dependencies.neuroimaging_cv_available and hasattr(cv_object, 'save_neuroimaging_results'):
    ...
    try:
        with memory_monitoring_context("Visualization"):
            if 'synthetic' in args.dataset:
                true_params = safe_pickle_load(results_dir / 'synthetic_data.dictionary', "Synthetic data")
                if true_params:
                    visualization.synthetic_data(str(results_dir), true_params, args, hypers)
            else:
...
def _create_brain_visualizations(args, results_dir, data, dependencies):
    ...
    if not (getattr(args, 'create_factor_maps', False) or 
    ...
    if not dependencies.factor_mapping_available:
        logging.warning("Brain visualization requested but factor mapping module not available")
        return
        ...
        with memory_monitoring_context("Brain visualization"):
            ...
            if rob_params and 'W' in rob_params:
                ...
                factor_maps = integrate_with_visualization(
                    str(results_dir), data, W, args.data_dir,
