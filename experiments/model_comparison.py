...
class ModelArchitectureComparison(ExperimentFramework):
    ...
    def _run_model_architecture(self, X_list: List[np.ndarray],
        ...
        try:
            ...
            self.logger.info(f"Training {model_name} with K={args.get('K', 5)}, "
            ...
            if len(potential_energy) > 0:
                ...
                self.logger.debug(f"Potential energy stats: mean={np.mean(potential_energy):.3f}")
            else:
            ...
            self.logger.info(f"{model_name} training completed in {elapsed:.2f}s, "
            ...
            result = {
                ...
                'Z_samples': Z_samples,
                'samples': samples,
                'log_likelihood': float(log_likelihood) if not np.isnan(log_likelihood) else float('-inf'),
    ...
    def _run_traditional_method(self, X: np.ndarray, method_name: str,
        ...
        try:
            ...
            if method_name == 'pca':
                ...
                method = FastICA(n_components=n_components, max_iter=1000, random_state=42)
                Z = method.fit_transform(X)
                W = method.components_.T
    ...
    def _analyze_model_architectures(self, results: Dict, performance_metrics: Dict) -> Dict:
        """Analyze model architecture comparison results."""
        analysis = {
            'summary': {
                'total_models_tested': len(results),
                'successful_models': sum(1 for r in results.values() if r.get('convergence', False)),
                'total_execution_time': sum(performance_metrics[m]['execution_time'] for m in performance_metrics)
            },
        ...
        for model_name, metrics in performance_metrics.items():
            analysis['performance_comparison'][model_name] = {
                ...
                'memory_efficiency': 1.0 / max(metrics['peak_memory_gb'], 0.1),  # Inverse for efficiency
                'convergence_success': metrics['convergence']
            }

        # Model quality comparison
        successful_models = [m for m in results.keys() if results[m].get('convergence', False)]
        if successful_models:
            log_likelihoods = {m: results[m].get('log_likelihood', float('-inf'))
                             for m in successful_models}

            # Find best model by log-likelihood
            if any(ll != float('-inf') and not np.isnan(ll) for ll in log_likelihoods.values()):
                best_model = max(log_likelihoods.keys(),
                               key=lambda x: log_likelihoods[x] if not np.isnan(log_likelihoods[x]) else float('-inf'))
                analysis['model_quality']['best_model'] = best_model
                analysis['model_quality']['log_likelihoods'] = log_likelihoods

        # Convergence analysis
        convergence_rates = {m: int(metrics['convergence']) for m, metrics in performance_metrics.items()}
    ...
    def _analyze_traditional_methods(self, results: Dict, performance_metrics: Dict) -> Dict:
        """Analyze traditional method results."""
        analysis = {
            'summary': {
                'methods_tested': len(results),
                'successful_methods': sum(1 for r in results.values() if r.get('convergence', False))
            },
            ...
            if result.get('convergence', False):
                if method_name == 'pca':
                    analysis['method_characteristics'][method_name] = {
                        'total_explained_variance': np.sum(result.get('explained_variance_ratio', [])),
                        'eigenvalue_decay': result.get('singular_values', [])
                    }
                elif method_name == 'fa':
                    analysis['method_characteristics'][method_name] = {
                        'log_likelihood': result.get('loglikelihood', float('-inf')),
                        'noise_variance_mean': np.mean(result.get('noise_variance', []))
                    }
    ...
    def _plot_model_comparison(self, results: Dict, performance_metrics: Dict) -> Dict:
        ...
        try:
            ...
            successful_models = [m for m in models if results[m].get('convergence', False)]

            if successful_models:
                # Execution time comparison
                times = [performance_metrics[m]['execution_time'] for m in successful_models]
                ...
                log_liks = [results[m].get('log_likelihood', float('-inf')) for m in successful_models]
                valid_log_liks = [(m, ll) for m, ll in zip(successful_models, log_liks)
                ...
                if valid_log_liks:
                    ...
                    axes[0, 1].text(0.5, 0.5, 'No valid log-likelihood data',
                                   ha='center', va='center', transform=axes[0, 1].transAxes)

                # Model complexity (placeholder)
                axes[1, 0].bar(successful_models, [self.comparison_params['K']] * len(successful_models))
                axes[1, 0].set_title('Model Complexity (K factors)')
                axes[1, 0].set_ylabel('Number of Factors')
                axes[1, 0].tick_params(axis='x', rotation=45)

                # Convergence status
                convergence = [int(results[m].get('convergence', False)) for m in models]
    ...
    def _plot_traditional_comparison(self, results: Dict, performance_metrics: Dict) -> Dict:
        ...
        try:
            ...
            successful_methods = [m for m in methods if results[m].get('convergence', False)]

            if successful_methods:
                # Execution time
                times = [performance_metrics[m]['execution_time'] for m in successful_methods]
                ...
                for method in successful_methods:
                    if method == 'pca':
                        score = np.sum(results[method].get('explained_variance_ratio', [0]))
                    elif method == 'fa':
...
def run_model_comparison(config=None, **kwargs):
    ...
    model_results = experiment.run_model_architecture_comparison(X_list, hypers, args, **kwargs)
