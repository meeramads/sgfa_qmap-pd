...
class ReproducibilityExperiments(ExperimentFramework):
    ...
    def _apply_data_perturbation(self, X_list: List[np.ndarray], 
                               perturbation_type: str, 
                               perturbation_level: float) -> List[np.ndarray]:
        ...
        for X in X_list:
            if perturbation_type == 'gaussian':
                ...
                if n_permute > 0:
                    feature_indices = np.random.choice(n_features, n_permute, replace=False)
    ...
    def _test_numerical_stability(self, X_list: List[np.ndarray], hypers: Dict, args: Dict, **kwargs) -> Dict:
    ...
    def _test_platform_consistency(self, X_list: List[np.ndarray], hypers: Dict, args: Dict, **kwargs) -> Dict:
    ...
    def _compute_result_checksums(self, results: Dict) -> Dict:
        ...
        for experiment_name, experiment_results in results.items():
            ...
            try:
                serialized = pickle.dumps(experiment_results, protocol=2)  # Use protocol 2 for consistency
                checksum = hashlib.md5(serialized).hexdigest()
                checksums[experiment_name] = checksum
                
            except Exception as e:
                self.logger.warning(f"Failed to compute checksum for {experiment_name}: {str(e)}")
    ...
    def _analyze_seed_reproducibility(self, results: Dict, performance_metrics: Dict) -> Dict:
        ...
        log_likelihoods = [performance_metrics[seed]['log_likelihood'] 
                          for seed in seeds if not np.isnan(performance_metrics[seed]['log_likelihood'])]
        convergences = [performance_metrics[seed]['convergence'] for seed in seeds]
        execution_times = [performance_metrics[seed]['execution_time'] for seed in seeds]
        ...
        if log_likelihoods:
            ...
            analysis['likelihood_variation'] = {
                ...
                'range': ll_range,
                'reproducibility_score': 1.0 / (1.0 + ll_std)  # Higher is more reproducible
            }
    ...
    def _analyze_data_perturbation_robustness(self, results: Dict, baseline_result: Dict) -> Dict:
        ...
        for perturbation_type, type_results in results.items():
            ...
            for level, level_results in type_results.items():
                level_likelihoods = [r['log_likelihood'] for r in level_results 
                ...
                if level_likelihoods:
                    ...
                    likelihood_drop = baseline_likelihood - mean_likelihood if not np.isnan(baseline_likelihood) else np.nan
                    
                    type_analysis[level] = {
                        'mean_likelihood': mean_likelihood,
                        'likelihood_drop': likelihood_drop,
                        'convergence_rate': np.mean(level_convergences),
                        'robustness_score': 1.0 / (1.0 + abs(likelihood_drop)) if not np.isnan(likelihood_drop) else 0.0
                    }
            
            analysis['robustness_by_perturbation_type'][perturbation_type] = type_analysis
        
        return analysis
    
    def _analyze_initialization_robustness(self, results: Dict, performance_metrics: Dict) -> Dict:
        ...
        for strategy, strategy_results in results.items():
            strategy_likelihoods = [r['log_likelihood'] for r in strategy_results 
                                  if not np.isnan(r['log_likelihood'])]
            strategy_convergences = [r['convergence'] for r in strategy_results]
            strategy_iterations = [r['n_iterations'] for r in strategy_results 
            ...
            if strategy_likelihoods:
                analysis['strategy_comparison'][strategy] = {
                    ...
                    'mean_iterations': np.mean(strategy_iterations) if strategy_iterations else np.nan,
                    'consistency_score': 1.0 / (1.0 + np.std(strategy_likelihoods))
                }
        
        # Find best strategy
        if analysis['strategy_comparison']:
            best_strategy = max(analysis['strategy_comparison'].keys(),
    ...
    def _analyze_computational_reproducibility(self, results: Dict) -> Dict:
        ...
        analysis['numerical_stability'] = {
            ...
            'stability_rate': len(stable_dtypes) / len(stability_results) if stability_results else 0.0
        }
        ...
        analysis['platform_consistency'] = {
            ...
            'consistency_rate': len(consistent_platforms) / len(platform_results) if platform_results else 0.0
        }
        
        return analysis
    
    def _plot_seed_reproducibility(self, results: Dict, performance_metrics: Dict) -> Dict:
        """Generate plots for seed reproducibility analysis."""
        plots = {}
        
        try:
            seeds = list(results.keys())
            log_likelihoods = [performance_metrics[seed]['log_likelihood'] for seed in seeds]
            execution_times = [performance_metrics[seed]['execution_time'] for seed in seeds]
            convergences = [performance_metrics[seed]['convergence'] for seed in seeds]
            ...
            if log_likelihoods:
                ...
                if valid_ll:
                    axes[0, 0].axhline(np.mean(valid_ll), color='red', linestyle='--', label='Mean')
                    axes[0, 0].legend()
            ...
            convergence_colors = ['red' if not conv else 'green' for conv in convergences]
            axes[1, 0].scatter(seeds, [1 if conv else 0 for conv in convergences], c=convergence_colors)
            ...
            if valid_likelihoods:
                axes[1, 1].hist(valid_likelihoods, bins=min(10, len(valid_likelihoods)), alpha=0.7)
            ...
            self.logger.warning(f"Failed to create seed reproducibility plots: {str(e)}")
    ...
    def _plot_data_perturbation_robustness(self, results: Dict) -> Dict:
        ...
        try:
            baseline_likelihood = results['baseline']['result'].get('log_likelihood', np.nan)
                    ...
                    for result in level_results:
                        if not np.isnan(result['log_likelihood']):
                            perturbation_data.append({
                                ...
                                'likelihood_drop': baseline_likelihood - result['log_likelihood'] if not np.isnan(baseline_likelihood) else np.nan
                            })
            ...
            for i, perturbation_type in enumerate(df['perturbation_type'].unique()):
                ...
                mean_likelihoods = [type_data[type_data['level'] == level]['log_likelihood'].mean() 
                                  for level in levels]
                
                axes[0, 0].plot(levels, mean_likelihoods, 'o-', label=perturbation_type)
            
            if not np.isnan(baseline_likelihood):
                axes[0, 0].axhline(baseline_likelihood, color='black', linestyle='--', label='Baseline')
            ...
            for perturbation_type in df['perturbation_type'].unique():
                type_data = df[df['perturbation_type'] == perturbation_type]
                levels = sorted(type_data['level'].unique())
                convergence_rates = [type_data[type_data['level'] == level]['convergence'].mean() 
                                   for level in levels]
                
                axes[0, 1].plot(levels, convergence_rates, 's-', label=perturbation_type)
            ...
            likelihood_drops = [df[df['perturbation_type'] == pt]['likelihood_drop'].dropna().values 
            ...
            pivot_data = df.pivot_table(values='log_likelihood', 
            ...
            self.logger.warning(f"Failed to create data perturbation robustness plots: {str(e)}")
        
        return plots
    
    def _plot_initialization_robustness(self, results: Dict, performance_metrics: Dict) -> Dict:
        ...
        try:
            ...
            likelihood_data = [df[df['strategy'] == s]['log_likelihood'].values for s in strategies]
            
            axes[0, 0].boxplot(likelihood_data, labels=strategies)
            axes[0, 0].set_ylabel('Log Likelihood')
            axes[0, 0].set_title('Log Likelihood Distribution by Initialization Strategy')
            axes[0, 0].tick_params(axis='x', rotation=45)
            axes[0, 0].grid(True, alpha=0.3)
            
            # Plot 2: Convergence rate by strategy
            convergence_rates = [df[df['strategy'] == s]['convergence'].mean() for s in strategies]
            ...
            if not iteration_data.empty:
                iteration_by_strategy = [iteration_data[iteration_data['strategy'] == s]['n_iterations'].values 
                                       for s in strategies if s in iteration_data['strategy'].values]
                strategy_labels = [s for s in strategies if s in iteration_data['strategy'].values]
            ...
            self.logger.warning(f"Failed to create initialization robustness plots: {str(e)}")
            ...
            if len(exact_results) > 1:
                ...
                if valid_likelihoods:
                    ...
                    if len(valid_likelihoods) > 1:
                        ...
                        axes[0, 0].text(0.05, 0.95, f'Std Dev: {std_dev:.2e}', 
