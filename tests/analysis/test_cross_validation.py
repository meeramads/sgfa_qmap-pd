...
class TestCVRunner:
    ...
    def test_check_cv_availability_success(self, sample_config, temp_dir):
        ...
        with patch("analysis.cross_validation.SparseBayesianGFACrossValidator"), patch(
            "analysis.cross_validation.CVConfig"
        ):
    ...
    def test_check_cv_availability_failure(self, sample_config, temp_dir):
        ...
        with patch(
            "analysis.cross_validation.SparseBayesianGFACrossValidator",
    ...
    def test_check_neuroimaging_cv_availability_success(self, sample_config, temp_dir):
        ...
        with patch("analysis.cross_validation.NeuroImagingCrossValidator"), patch(
            "analysis.cross_validation.NeuroImagingCVConfig"
    ...
    def test_check_neuroimaging_cv_availability_failure(self, sample_config, temp_dir):
        ...
        with patch(
            "analysis.cross_validation.NeuroImagingCrossValidator",
            side_effect=ImportError,
        ):
            result = cv_runner._check_neuroimaging_cv_availability()
            assert result is False
    ...
    def test_run_cv_analysis_no_modules(
        self,
        mock_logger,
        sample_config,
        temp_dir,
        ...
        with patch.object(
            cv_runner, "_check_cv_availability", return_value=False
        ), patch.object(
            cv_runner, "_check_neuroimaging_cv_availability", return_value=False
        ):
            ...
            mock_logger.error.assert_called_with(
                "Cross-validation requested but no CV module available!"
            )

    def test_run_cv_analysis_neuroimaging_cv(
        self, sample_config, temp_dir, sample_synthetic_data, sample_hyperparameters
    ):
        ...
        with patch.object(
            cv_runner, "_check_neuroimaging_cv_availability", return_value=True
        ), patch.object(
            cv_runner,
            "_run_neuroimaging_cv_analysis",
            return_value=(mock_results, mock_cv_object),
        ):
    ...
    def test_run_cv_analysis_basic_cv(
        self, sample_config, temp_dir, sample_synthetic_data, sample_hyperparameters
    ):
        ...
        with patch.object(
            cv_runner, "_check_cv_availability", return_value=True
        ), patch.object(
            cv_runner, "_check_neuroimaging_cv_availability", return_value=False
        ), patch.object(
            cv_runner,
    ...
    def test_run_neuroimaging_cv_analysis_standard(
        self, sample_config, temp_dir, sample_synthetic_data, sample_hyperparameters
    ):
        ...
        with patch(
            "analysis.cross_validation.NeuroImagingCrossValidator", return_value=mock_cv
        ), patch(
            "analysis.cross_validation.NeuroImagingCVConfig"
        ) as mock_config_cls, patch(
            "analysis.cross_validation.ParkinsonsConfig"
        ) as mock_pd_config_cls:
    ...
    def test_run_neuroimaging_cv_analysis_nested(
        self, sample_config, temp_dir, sample_synthetic_data, sample_hyperparameters
    ):
        ...
        with patch(
            "analysis.cross_validation.NeuroImagingCrossValidator", return_value=mock_cv
        ), patch("analysis.cross_validation.NeuroImagingCVConfig"), patch(
            "analysis.cross_validation.ParkinsonsConfig"
        ):
    ...
    def test_run_basic_cv_analysis(
        self, sample_config, temp_dir, sample_synthetic_data, sample_hyperparameters
    ):
        """Test basic CV analysis."""
        ...
        with patch(
            ...
            mock_cv.standard_cross_validate.assert_called_once_with(
                sample_synthetic_data["X_list"], sample_config, sample_hyperparameters
            )
