...
class TestModels:
    ...
    def test_models_basic_structure(
        self, sample_synthetic_data, sample_hyperparameters
    ):
    ...
    def test_models_parameter_validation(self):
        ...
        with pytest.raises(
            AssertionError, match="Number of data sources does not match"
        ):
            models(X_list, hypers, args)
...
class TestRunInference:
    ...
    def test_run_inference_basic(
        ...
        mock_mcmc.assert_called_once_with(
            mock_nuts_instance, num_samples=100, num_chains=2, num_warmup=50
        )
    ...
    def test_run_inference_logging(
        self, mock_logger, sample_synthetic_data, sample_hyperparameters
    ):
        ...
        with patch("run_analysis.MCMC") as mock_mcmc, patch("run_analysis.NUTS"):
    ...
    def test_run_inference_device_handling(
        self, sample_synthetic_data, sample_hyperparameters
    ):
        ...
        with patch("run_analysis.jax.devices") as mock_devices, patch(
            "run_analysis.MCMC"
        ) as mock_mcmc, patch("run_analysis.NUTS"):
            ...
            mock_mcmc_instance = Mock()
            mock_mcmc_instance.get_samples.return_value = {
                "Z": np.random.normal(0, 1, (10, 50, 5))
            }
            mock_mcmc.return_value = mock_mcmc_instance
...
class TestMainFunction:
    ...
    def test_main_synthetic_analysis(self, temp_dir):
        ...
        with patch("run_analysis.validate_and_setup_args", return_value=args), patch(
            "run_analysis.run_inference", return_value=mock_mcmc_results
        ), patch("run_analysis._create_visualizations"), patch(
            "run_analysis._log_final_summary"
        ):
    ...
    def test_main_with_cv(self, temp_dir):
        ...
        with patch("run_analysis.validate_and_setup_args", return_value=args), patch(
            "run_analysis.should_run_cv_analysis", return_value=True
        ), patch(
            "run_analysis.should_run_standard_analysis", return_value=False
        ), patch(
            "run_analysis.CVRunner"
        ) as mock_cv_runner:
