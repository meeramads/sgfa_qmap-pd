...
class TestFullPipeline:
    ...
    def test_cv_pipeline(self, temp_dir, sample_synthetic_data):
        ...
        with patch.object(CVRunner, '_check_cv_availability', return_value=True), \
             patch.object(CVRunner, '_run_basic_cv_analysis', return_value=(mock_cv_results, mock_cv_object)):
    ...
    def test_pipeline_with_different_configs(self, temp_dir):
        ...
        for config_params in configs_to_test:
            ...
            n_features = sum([60//config_params['num_sources']] * config_params['num_sources'])
            mock_results = {
...
class TestPipelineDataFlow:
    ...
    def test_data_format_consistency(self, temp_dir):
        ...
        assert all(X.shape[0] == X_list[0].shape[0] for X in X_list)  # Same number of subjects
    ...
    def test_logging_integration(self, mock_cv_logger, mock_model_logger, mock_data_logger, temp_dir):
