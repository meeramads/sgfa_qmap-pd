...
class ModelRunner:
    ...
    def run_standard_analysis(self, X_list: List[np.ndarray], 
                            hypers: Dict, data: Dict) -> Dict:
        ...
        for run_id in range(1, self.config.num_runs + 1):
            ...
            try:
                run_results = self._run_single_mcmc(
                    X_list, hypers, run_id
                )
                results[run_id] = run_results
                
                # Process robust parameters if multi-chain
                if self.config.num_chains > 1:
                    robust_params = self._extract_robust_parameters(
                        run_results
                    )
    ...
    def _run_single_mcmc(self, X_list: List[np.ndarray], 
                        hypers: Dict, run_id: int) -> Dict:
            ...
            if self.config.num_chains > 1:
                ...
                if success:
                    ...
                    robust_params.update({
                        ...
                        'n_robust_components': robust_params.get('W', np.array([])).shape[1] if 'W' in robust_params else 0
                    })
                    
                    # Add model-specific parameters
                    if hasattr(self.config, 'model') and 'sparseGFA' in self.config.model:
                    ...
                    return {'extraction_successful': False, 'reason': 'no_robust_components'}
            else:
                ...
                if W is not None and Z is not None:
                    ...
                    return {'extraction_successful': False, 'reason': 'missing_parameters'}
                
        except Exception as e:
            logger.error(f"Robust parameter extraction failed: {e}")
            return {'extraction_successful': False, 'reason': f'extraction_error: {str(e)}'}
