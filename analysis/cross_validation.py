...
class CVRunner:
    ...
    def _run_basic_cv_analysis(self, X_list, hypers, data):
        ...
        from .cross_validation_library import CVConfig, SparseBayesianGFACrossValidator
...
def should_run_cv_analysis(config):
    ...
    return (
        getattr(config, "run_cv", False)
        or getattr(config, "cv_only", False)
        or getattr(config, "neuroimaging_cv", False)
    )
