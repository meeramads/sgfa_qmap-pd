...
def apply_comprehensive_cv_framework(X_list: List[np.ndarray], config: Dict,
                                   optimal_params: Dict, data_dir: str) -> Tuple[Dict, Dict]:
    """
    ...
    try:
        ...
        logger.info(f"CV framework initialized with {cv_config.outer_cv_folds} outer folds")
        ...
        # Run comprehensive cross-validation (nested CV with hyperparameter optimization)
        ...
        comprehensive_cv_results = {
            ...
            'optimization_improvements': _compare_with_manual_optimization(cv_results, optimal_params)
        }
...
def _create_cv_configuration(config: Dict, has_clinical_data: bool) -> Any:
    """Create comprehensive CV configuration."""
    try:
        from analysis.cross_validation_library import NeuroImagingCVConfig, ParkinsonsConfig
        ...
        if has_clinical_data:
            ...
            cv_config.clinical_stratification_vars = pd_config.clinical_stratification_vars
        ...
        cv_config.n_jobs = min(config.get('system', {}).get('num_workers', 4), 4)  # Limit parallelization
        cv_config.memory_efficient = True
        cv_config.enable_early_stopping = True

        logger.info(f"CV configuration created: {cv_config.outer_cv_folds} outer × {cv_config.inner_cv_folds} inner folds")
...
def _load_clinical_data(data_dir: str) -> Optional[Any]:
    ...
    try:
        ...
        if 'clinical_data' in data and data['clinical_data'] is not None:
            ...
            logger.info(f"Clinical data loaded: {clinical_data.shape[0]} subjects, {clinical_data.shape[1]} variables")
...
def _define_hyperparameter_space(config: Dict, current_optimal: Dict) -> List[Dict]:
    ...
    try:
        ...
        K_expanded = list(set(K_candidates + [max(5, current_K-2), current_K, min(20, current_K+2)]))
        ...
        percW_expanded = list(set(percW_candidates + [max(15, current_percW-5), current_percW, min(60, current_percW+5)]))
                ...
                for num_samples in num_samples_candidates:
                    for num_chains in num_chains_candidates:
                        param_combination = {
                            ...
                            'target_accept_prob': current_optimal.get('target_accept_prob', 0.8)
                        }
        ...
        for param in ['K', 'percW', 'num_samples', 'num_chains']:
            if param in cv_params and param in manual_optimal:
                if cv_params[param] != manual_optimal[param]:
                    comparison['improvements'].append(f"{param}: {manual_optimal[param]} → {cv_params[param]}")
...
def _fallback_cv_analysis(X_list: List[np.ndarray], config: Dict, optimal_params: Dict) -> Tuple[Dict, Dict]:
    ...
    try:
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(kf.split(range(n_subjects))):
            try:
                ...
                reconstruction_quality = _assess_reconstruction_quality(X_train, X_val, optimal_params)
                cv_scores.append(reconstruction_quality)

                logger.info(f"   Fold {fold_idx + 1}: score = {reconstruction_quality:.4f}")
        ...
        logger.info(f"✅ Fallback CV completed: {mean_cv_score:.4f} ± {std_cv_score:.4f}")

        return fallback_results, optimal_params

    except Exception as e:
        logger.error(f"❌ Fallback CV analysis failed: {e}")
        return {'cv_framework_used': False, 'status': 'failed', 'error': str(e)}, optimal_params


def _assess_reconstruction_quality(X_train: List[np.ndarray], X_val: List[np.ndarray], params: Dict) -> float:
...
def integrate_cv_with_pipeline(X_list: List[np.ndarray], config: Dict,
                             current_optimal_params: Dict, data_dir: str) -> Tuple[Dict, Dict, Dict]:
    """
    ...
    try:
        ...
        if integration_summary['parameter_enhancement']:
            ...
            for key in enhanced_params:
                if key in current_optimal_params and enhanced_params[key] != current_optimal_params[key]:
                    param_changes.append(f"{key}: {current_optimal_params[key]} → {enhanced_params[key]}")
            integration_summary['parameter_changes'] = param_changes

        logger.info("✅ CV framework pipeline integration completed")
        if integration_summary['parameter_enhancement']:
            logger.info(f"   Parameters enhanced by CV: {integration_summary.get('parameter_changes', [])}")
        else:
        ...
        return {'status': 'failed', 'error': str(e)}, current_optimal_params, fallback_summary