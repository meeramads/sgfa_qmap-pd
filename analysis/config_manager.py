...
class ConfigManager:
    ...
    def setup_analysis_config(self) -> AnalysisConfig:
        ...
        logging.info(f"Analysis plan: Standard={self.config.run_standard}, CV={self.config.run_cv}")
    ...
    def _should_run_cv_analysis(self) -> bool:
        ...
        return (getattr(self.args, 'run_cv', False) or 
                getattr(self.args, 'cv_only', False) or 
                getattr(self.args, 'neuroimaging_cv', False))
    ...
    def _create_result_directories(self):
        ...
        if self.config.run_standard:
            self.config.standard_res_dir = create_results_structure(
                self.results_base, self.args.dataset, self.args.model, 
                self.config.flag, self.config.flag_regZ
            )
        ...
        if self.config.run_cv:
            ...
            self.config.cv_res_dir = create_results_structure(
                self.results_base, f"{self.args.dataset}_cv", self.args.model,
                self.config.flag, f"{self.config.flag_regZ}{cv_suffix}"
            )
    ...
    def get_hyperparameters_dir(self) -> Path:
        ...
        return self.config.standard_res_dir if self.config.run_standard else self.config.cv_res_dir
